"use strict";
let intentosFallidos = 0;

document.addEventListener("DOMContentLoaded", () => {
    // Si ya hay sesión, redirigir directo
    if (SGG.obtenerSesion()) window.location.href = "dashboard.html";

    const loginForm = document.getElementById("login-form");
    if(loginForm) {
        loginForm.addEventListener("submit", (e) => {
            e.preventDefault();
            const btn = document.getElementById("btn-submit-login");
            const identifier = document.getElementById("login-identifier").value.trim().toLowerCase();
            const password = document.getElementById("login-password").value;
            
            const users = JSON.parse(localStorage.getItem("usuarios_sgg")) || [];
            const user = users.find(u => u.username.toLowerCase() === identifier || u.email.toLowerCase() === identifier);

            if (user && user.password === password) {
                intentosFallidos = 0;
                SGG.crearSesion(user); // Guarda la sesión global
                showMessage("login-msg", `¡Bienvenido/a, ${user.nombre}! Redirigiendo...`, "success");
                setTimeout(() => { window.location.href = "dashboard.html"; }, 1500);
            } else {
                intentosFallidos++;
                if (intentosFallidos >= 3) {
                    btn.disabled = true;
                    let timeLeft = 30;
                    const interval = setInterval(() => {
                        timeLeft--;
                        showMessage("login-msg", `Bloqueo de seguridad. Espera ${timeLeft}s.`, "error");
                        if (timeLeft <= 0) {
                            clearInterval(interval);
                            btn.disabled = false;
                            intentosFallidos = 0;
                            document.getElementById("login-msg").classList.remove("show");
                        }
                    }, 1000);
                } else {
                    showMessage("login-msg", "Credenciales incorrectas.", "error");
                }
            }
        });
    }
});
